...
add_action(
	VDZ_YS_API,
	function( $action = '', $plugin_name = '' ) {
		if ( empty( $plugin_name ) ) {
			$plugin_name = plugin_basename( __FILE__ );
		}
		if ( empty( $action ) || ! in_array( $action, array( 'on', 'off' ), true ) ) {
			$action = 'on'; // on || off
		}
		...
		if ( ! is_wp_error( $response ) && isset( $response['body'] ) && ( json_decode( $response['body'] ) !== null ) ) {
			...
			set_transient( VDZ_YS_API . '_activation_notice_text', $response['body'], 60 );
		}
	},
	10,
	2
...
add_action(
	...
	function () {
		...
		$message     = get_transient( VDZ_YS_API . '_activation_notice_text' );
		...
		if ( ! empty( $all_plugins ) ) {
			...
			foreach ( $all_plugins as $link => $plugin_name ) {
				...
				$url_encode_plugin_name = rawurlencode( '"' . trim( $plugin_name ) . '"' );
			...
			printf( '<div id="' . esc_attr( VDZ_YS_API ) . '" class="%1$s">%2$s</div>', esc_attr( $class ), wp_kses_post( $msg ) );
			// Мгновенное удаление временного сообщения
			delete_transient( VDZ_YS_API . '_activation_notice_text' );
		}
	}
);
